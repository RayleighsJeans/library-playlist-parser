...
class MetadataAggregator:
    ...
    def get_cover_art(self, artist: str, album: str, title: str = '', metadata_source: str = '') -> Optional[bytes]:
        """Get cover art with fallback: MusicBrainz -> Last.fm -> SoundCloud."""
...
class MusicTagger:
    ...
    def tag_file(self, filepath: str, output_dir: str) -> bool:
        ...
        if success:
            # Copy to sorted directory (instead of move)
...
def main():
    ...
    parser.add_argument('--library', default='soundcloud/library.txt', help='Path to library.txt')
    parser.add_argument('--input-dir', default='soundcloud', help='Input directory with music files')
    parser.add_argument('--output-dir', default='soundcloud/sorted', help='Output directory for tagged files')
    ...
    music_files = [f for f in music_files if 'sorted' not in str(f)]
